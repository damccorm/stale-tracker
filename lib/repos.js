...
function processGitHubRepo(repo) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        pullRequests.forEach(pull => {
            ...
            const curPr = { author: pull.user.login, timeSinceUpdate: duration, link: `${repo.url}/pull/${pull.number}` };
            let isStale = false;
                            ...
                            for (let l = 0; l < (group.azpReviewers || []).length; l++) {
                                ...
                                if (!isStale) {
                                    if ((reviewer.displayName || '').toLowerCase().indexOf(azpReviewer.toLowerCase()) > -1) {
                                        ...
                                        duration = duration / 1000 / 60 / 60; //Duration in h.
                                        const formattedPull = { author: author, timeSinceUpdate: duration, link: getPullRequestUrl(repoUrl, pull.pullRequestId) };
